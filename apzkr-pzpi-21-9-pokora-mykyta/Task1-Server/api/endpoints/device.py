...
from schemas.food_patch_schemas import FoodPatchCreate, FoodPatchResponse
...
@device_router.post("/{aquarium_id}/food-patch", response_model=FoodPatchResponse,
                    summary="Наповнення порції для пристрою акваріума")
@require_permissions("manage_devices")
async def fill_food_patch(
        aquarium_id: int = Path(..., description="ID акваріума"),
        food_patch_data: FoodPatchCreate = ...,
        current_user: dict = Depends(get_current_user),
        device_service: DeviceFeedingService = Depends(get_device_feeding_service),
        role_manager: RoleManager = Depends(get_role_manager)
):
    try:
        filled_patch = device_service.fill_food_patch(aquarium_id, food_patch_data)
        return FoodPatchResponse.from_orm(filled_patch)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@device_router.delete("/{aquarium_id}/food-patch", summary="Видалення порції для пристрою акваріума")
@require_permissions("manage_devices")
async def delete_food_patch(
        aquarium_id: int = Path(..., description="ID акваріума"),
        current_user: dict = Depends(get_current_user),
        device_service: DeviceFeedingService = Depends(get_device_feeding_service),
        role_manager: RoleManager = Depends(get_role_manager)
    ...
    try:
        device_service.delete_food_patch(aquarium_id)
        return {"message": "FoodPatch успішно видалено"}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
